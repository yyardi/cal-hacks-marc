...
"""
Automated Workspace Center Finder
...
def scan_workspace_grid(robot, svg_file, analysis, page_width_mm, page_height_mm,
                       z_contact, pitch=0.0,
                       x_min=0.17, x_max=0.2, x_step=0.01,
                       y_min=-0.045, y_max=0.05, y_step=0.001):
    """
    ...
    for center_x in x_positions:
        for center_y in y_positions:
            ...
            if test_num % 5 == 0 or test_num == total_tests:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python find_optimal_center.py <svg_file> [page_width_mm] [page_height_mm]")
        print("\nExample:")
        print("  python find_optimal_center.py butterfly.svg")
        print("  python find_optimal_center.py butterfly.svg 70 70")
        sys.exit(1)
    ...
    # Get parameters
    z_contact_input = input("\nEnter z_contact (meters, or Enter for 0.0): ").strip()
    ...
    print("  Y: [-0.09, 0.09] step 0.01 (19 positions)")
    print("  Total: 171 tests")
    
    use_default = input("\nUse default search area? (yes/no): ")
    
    if use_default.lower() in ['yes', 'y']:
        x_min, x_max, x_step = 0.17, 0.25, 0.01
        y_min, y_max, y_step = -0.09, 0.09, 0.01
    else:
    ...
    if results:
        ...
        if response.lower() in ['yes', 'y']:
            ...
            print("\n✓ Calibration saved!")
            print("  You can now use auto_scale_svg_calibrated.py with this center")
    else:
