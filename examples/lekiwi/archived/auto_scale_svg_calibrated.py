...
"""
SVG Auto-Scaler with Workspace Calibration

This version uses saved workspace calibration to properly center drawings.
Run workspace calibration once, then all future drawings will be centered correctly.
"""
...
from workspace_calibration import (
    ...
    calibrate_workspace_bounds_from_center
)
...
def analyze_svg(svg_file):
    """Analyze SVG and recommend drawing size."""
    print("\n" + "="*70)
    print("ANALYZING SVG")
    print("="*70)
    ...
    # Recommend 70mm as default (safe size for most workspaces)
    recommended_width = 70.0
    recommended_height = recommended_width / aspect_ratio
    
    print(f"\nRecommended drawing size:")
    print(f"  Width: {recommended_width:.1f} mm")
    print(f"  Height: {recommended_height:.1f} mm")
...
def draw_scaled_svg_calibrated(svg_file, controller, analysis, z_contact, z_safe, 
                               pitch=0.0, min_threshold=0.90,
                               calibrated_center_x=None, calibrated_center_y=None):
    """
    Draw SVG using calibrated center position.
    ...
    page_width_mm, page_height_mm = analysis['recommended']
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python auto_scale_svg_calibrated.py <svg_file>")
        sys.exit(1)
    ...
    if calibration:
        ...
        response = input("Recalibrate workspace center? (yes/no): ")
        if response.lower() in ['yes', 'y']:
            calibration = None
    
    if not calibration:
        print("\n⚙️  Starting workspace calibration...")
        center_x, center_y, z_cal = interactive_center_calibration(
            controller,
            initial_x=0.21,
            initial_y=0.0,
            initial_z=0.0,
        ...
        # Save calibration
        save_calibration(center_x, center_y, z_cal)
        calibration = {
            'center_x': center_x,
            'center_y': center_y,
            'z_contact': z_cal
