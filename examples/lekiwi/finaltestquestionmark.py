...
"""
SVG Auto-Scaler for Robot Workspace (CENTERED VERSION with Z-CALIBRATION)

This version automatically centers the drawing in the robot's workspace
...
def calculate_centered_origin(page_width_mm, page_height_mm, 
                              workspace_x_min=0.17, workspace_x_max=0.25,
                              workspace_y_min=-0.09, workspace_y_max=0.09):
    """
    Calculate robot origin to center the drawing in the REACHABLE workspace.
    
    NOTE: The actual reachable area is NOT the full geometric workspace!
    The arm struggles to reach x < 0.17 due to joint limits and geometry.
    
    Args:
        page_width_mm: Drawing width in mm
        page_height_mm: Drawing height in mm
        workspace_x_min: Min X coordinate of REACHABLE workspace (m) - DEFAULT 0.17
        workspace_x_max: Max X coordinate of REACHABLE workspace (m) - DEFAULT 0.25
        workspace_y_min: Min Y coordinate of REACHABLE workspace (m) - DEFAULT -0.09
        workspace_y_max: Max Y coordinate of REACHABLE workspace (m) - DEFAULT 0.09
    
    Returns:
        (robot_origin_x, robot_origin_y): Centered origin coordinates in meters
    """
    # Convert drawing size to meters
    page_width_m = page_width_mm * 0.001
    page_height_m = page_height_mm * 0.001
    
    # Calculate REACHABLE workspace center (shifted right from geometric center)
    workspace_center_x = (workspace_x_min + workspace_x_max) / 2
    workspace_center_y = (workspace_y_min + workspace_y_max) / 2
    
    # Calculate drawing center relative to origin
    drawing_center_offset_x = page_width_m / 2
    drawing_center_offset_y = page_height_m / 2
    
    # Calculate origin that centers the drawing in REACHABLE area
...
def interactive_z_calibration(controller, first_x, first_y, initial_z_contact):
    """
    Move to first waypoint and allow interactive z-height adjustment.
    
    Args:
        controller: RobotMovementController instance
        first_x: X coordinate of first drawing point
        first_y: Y coordinate of first drawing point
        initial_z_contact: Initial z_contact value
    
    Returns:
        float: Final calibrated z_contact value
    """
    print("\n" + "="*70)
    ...
    print("You can then adjust the z-height up/down until it's perfect.")
    ...
    print(f"Initial z_contact: {current_z:.3f} m")
    ...
    # Move to first point at current z
    print(f"\nMoving to position...")
    ...
    if not success:
        print("❌ Cannot reach first waypoint with IK!")
        print("Suggestion: Adjust drawing position or initial z_contact")
        return initial_z_contact
    ...
    print(f"✓ Robot at first waypoint")
    print(f"📍 Position: x={first_x:.3f}, y={first_y:.3f}, z={current_z:.3f}")
    
    # Interactive adjustment loop
    print("\n" + "="*70)
    print("ADJUST Z-HEIGHT")
    print("="*70)
    print("\nControls:")
    print("  'w' = Move UP by 1mm (increase z)")
    print("  's' = Move DOWN by 1mm (decrease z)")
    print("  'W' = Move UP by 5mm")
    print("  'S' = Move DOWN by 5mm")
    print("  'q' = Quit and use this height")
    print("\n⚠️  Watch the marker tip - adjust until it just touches the paper")
    print("⚠️  Current z =", f"{current_z:.3f} m ({current_z*1000:.1f} mm)")
    
    while True:
        command = input("\nCommand (w/s/W/S/q): ").strip()
        
        if command == 'q':
            print(f"\n✓ Z-height calibration complete!")
            print(f"  Final z_contact = {current_z:.3f} m ({current_z*1000:.1f} mm)")
            break
        
        elif command == 'w':
            current_z += z_increment
            print(f"Moving UP to z = {current_z:.3f} m ({current_z*1000:.1f} mm)")
        
        elif command == 's':
            current_z -= z_increment
            print(f"Moving DOWN to z = {current_z:.3f} m ({current_z*1000:.1f} mm)")
            ...
            print("Invalid command. Use w/s/W/S/q")
            continue
        
        # Update position with new z
        if command != 'q':
            success, joint_angles = xyzp_inverse_kinematics(
                controller.robot, first_x, first_y, current_z, 0.0
            )
            ...
            if not success:
                print("⚠️  Warning: Cannot reach this z position with IK!")
                print("Reverting to previous z")
                if command in ['w', 's']:
...
def safe_marker_pickup(robot, controller):
    ...
    print("\n✓ Ready for z-height calibration and drawing!")
    return True
...
def analyze_svg(svg_file):
    ...
    # Robot workspace - ACTUAL REACHABLE AREA (not geometric workspace)
    print(f"\nRobot Workspace:")
    print(f"  Geometric: X=[0.15, 0.25] Y=[-0.10, 0.10]")
    print(f"  REACHABLE: X=[0.17, 0.25] Y=[-0.09, 0.09] ⚠️")
    print(f"  Note: Arm cannot reach x < 0.17 reliably due to joint limits")
    print(f"  Safe drawing area: ~50mm × 120mm (centered in reachable zone)")
    
    # Calculate scaling options - MORE CONSERVATIVE for actual reachability
    aspect_ratio = svg_width / svg_height if svg_height > 0 else 1.0
    
    print(f"\nSVG Aspect Ratio: {aspect_ratio:.2f}")
    
    # Use VERY conservative sizes to ensure everything is reachable
    target_width_mm_1 = 50  # Much more conservative (was 70)
    scale_1 = target_width_mm_1 / svg_width if svg_width > 0 else 1
    scaled_height_1 = svg_height * scale_1
    
    target_height_mm_2 = 100  # More conservative (was 140)
    ...
    scale_x = 50 / svg_width if svg_width > 0 else 1
    scale_y = 100 / svg_height if svg_height > 0 else 1
    scale_3 = min(scale_x, scale_y)
    scaled_width_3 = svg_width * scale_3
    scaled_height_3 = svg_height * scale_3
    
    print(f"\n" + "="*70)
    print("SCALING OPTIONS (based on REACHABLE area)")
    print("="*70)
    
    print(f"\nOption 1: Fit to X-axis (width)")
    print(f"  page_width_mm = {target_width_mm_1}")
    print(f"  page_height_mm = {scaled_height_1:.1f}")
    print(f"  Fits: {'✓ Yes' if scaled_height_1 <= 100 else '✗ Too tall'}")
    
    print(f"\nOption 2: Fit to Y-axis (height)")
    print(f"  page_width_mm = {scaled_width_2:.1f}")
    print(f"  page_height_mm = {target_height_mm_2}")
    print(f"  Fits: {'✓ Yes' if scaled_width_2 <= 50 else '✗ Too wide'}")
    
    print(f"\nOption 3: Best fit (recommended) - CENTERED in REACHABLE area")
    print(f"  page_width_mm = {scaled_width_3:.1f}")
    print(f"  page_height_mm = {scaled_height_3:.1f}")
    print(f"  Fits: ✓ Yes (centered in actually reachable workspace)")
    print(f"  Note: Smaller sizes ensure >90% reachability")
...
def load_and_scale_svg(svg_file, actual_bounds, page_width_mm, page_height_mm,
    ...
    print(f"  Drawing will be CENTERED in workspace")
...
def draw_scaled_svg_robust(svg_file, controller, analysis_result, z_contact=0.0, z_safe=0.05,
    ...
    AUTOMATICALLY CENTERS the drawing in the workspace.
    ...
    # CALCULATE CENTERED ORIGIN in REACHABLE workspace
    robot_origin_x, robot_origin_y = calculate_centered_origin(
        page_width_mm, 
        page_height_mm,
        workspace_x_min=0.17,  # ACTUAL reachable area (not 0.15)
    ...
    print(f"\n" + "="*70)
    print("CENTERED DRAWING PLACEMENT (in REACHABLE area)")
    print("="*70)
    print(f"\nDrawing size: {page_width_mm:.1f} × {page_height_mm:.1f} mm")
    print(f"Geometric workspace: X=[0.15, 0.25] Y=[-0.10, 0.10]")
    print(f"REACHABLE workspace: X=[0.17, 0.25] Y=[-0.09, 0.09] ⚠️")
    print(f"Calculated origin: X={robot_origin_x:.3f} Y={robot_origin_y:.3f}")
    print(f"→ Drawing centered in REACHABLE zone (shifted right)")
    print(f"→ Note: Arm cannot reliably reach x < 0.17 due to joint limits")
    
    print(f"\nLoading SVG with centered placement...")
    ...
    robot_paths = load_and_scale_svg(
        ...
        page_width_mm,
        page_height_mm,
        robot_origin_x=robot_origin_x,  # USE CENTERED ORIGIN
        robot_origin_y=robot_origin_y   # USE CENTERED ORIGIN
    )
    ...
    # Interactive z-calibration at first waypoint
    calibrated_z_contact = interactive_z_calibration(
        controller, first_x, first_y, z_contact
    )
    
    # Update z_contact with calibrated value
    z_contact = calibrated_z_contact
    z_safe = z_contact + 0.05
    
    print(f"\nUsing calibrated z_contact = {z_contact:.3f} m for drawing")
    
    # First pass: validate reachability with calibrated z
    ...
    if reachable_fraction < min_threshold:
        ...
        print(f"\nSuggestions:")
        print(f"  1. Make the drawing smaller (current: {page_width_mm:.0f}×{page_height_mm:.0f}mm)")
        print(f"  2. Adjust z_contact higher (current: {z_contact:.3f}m)")
        print(f"  3. Drawing is already centered - may need smaller size")
        
        response = input("\nContinue anyway? (y/n): ")
        if response.lower() != 'y':
            return False
    
    # Final confirmation before drawing
    print("\n" + "="*70)
    print("⚠️  READY TO START DRAWING")
    print("="*70)
    print(f"\nDrawing parameters:")
    print(f"  z_contact = {z_contact:.3f} m ({z_contact*1000:.1f} mm)")
    print(f"  z_safe = {z_safe:.3f} m ({z_safe*1000:.1f} mm)")
    print(f"  Coverage = {reachable_fraction*100:.1f}%")
    print(f"  Paths = {len(robot_paths)}")
    print(f"  Origin = ({robot_origin_x:.3f}, {robot_origin_y:.3f}) - CENTERED")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python auto_scale_svg_centered.py <svg_file>")
        print("\nExample:")
        print("  python auto_scale_svg_centered.py butterfly.svg")
        sys.exit(1)
    ...
    print(f"\n💡 Drawing will be automatically centered in workspace")
    ...
    # Get z_contact (initial estimate)
    z_contact_input = input("\nEnter initial z_contact estimate (meters, or press Enter for 0.0): ").strip()
    z_contact = float(z_contact_input) if z_contact_input else 0.0
    z_safe = z_contact + 0.05
    
    print("\n💡 Note: You will be able to fine-tune z_contact interactively at the first waypoint")
    ...
    # Draw (includes z-calibration and auto-centering)
    try:
